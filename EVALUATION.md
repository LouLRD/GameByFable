...
## Commandes exécutées (passe mobile, 4 septembre 2026)

| Commande                                                    | Résultat exact                                                                                                                                                                                                                                             |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm ci`                                                    | 396 paquets installés, aucune erreur                                                                                                                                                                                                                       |
| `npm run check` (typecheck + lint + format + tests + build) | 49 fichiers de test, 496 tests verts ; build statique `dist/` en 4,2 s                                                                                                                                                                                     |
| `npm run test:coverage`                                     | 496 tests ; couverture V8 (domaine, scénario, persistance, état) : 93,45 % instructions, 80,61 % branches, 95,21 % fonctions, 95,55 % lignes                                                                                                               |
| `npm run validate:scenario`                                 | scénario « La Veilleuse » valide (9 zones, 6 personnages, 18 faits, 21 hypothèses, 5 fins), 2 avertissements d'exclusion asymétrique rendue symétrique                                                                                                     |
| `npm run walkthroughs`                                      | quatre parcours rejoués : `ending_transparent`, `ending_protective`, `ending_scapegoat`, `ending_procedural`                                                                                                                                               |
| `npm run test:e2e`                                          | 27 parcours verts, 6 ignorés par conception (raccourcis desktop sur mobile/tablette, parcours mobile sur desktop), 0 échec — projets `desktop` 1440 × 900, `mobile` 390 × 844, `mobile-small` 320 × 568, `mobile-landscape` 844 × 390, `tablet` 768 × 1024 |

Inspection visuelle réalisée par captures Playwright (`chromium`, émulation tactile) à 320 × 568, 360 × 800, 390 × 844, 412 × 915, 430 × 932, 844 × 390 et 768 × 1024 pour chaque espace et pour les feuilles (fiche de zone, formulaire d'hypothèse, détail de contradiction, confrontation, sauvegardes, table ronde), plus le bureau à 1440 × 900.

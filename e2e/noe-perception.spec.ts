...
test.describe('Parcours 2 — une perception n’est pas un fait', () => {
  test('la déclaration de Noé est reformulée sans que sa perception soit effacée', async ({ page }) => {
    test.setTimeout(120_000);
    await openFreshGame(page);
    // Examiner la déclaration de Noé
    await page.getByRole('toolbar', { name: 'Filtrer le dossier' }).getByRole('button', { name: /^Déclarations/ }).click();
    await page.locator('.casefile-item').filter({ hasText: 'Inès est entrée dans la réserve' }).first().click();
    ...
    for (let i = 0; i < 3; i += 1) await page.getByRole('button', { name: 'Avancer de 10 secondes' }).click();

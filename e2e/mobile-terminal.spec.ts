...
test.describe('Terminal mobile — parcours de bout en bout', () => {
  test('onboarding, preuve, plan, temps, hypothèse, contradiction, confrontation, sauvegardes', async ({
    page,
  }) => {
    ...
    expect(await page.getByRole('tab', { name: 'Version' }).getAttribute('aria-selected')).toBe(
      'true',
    );
    ...
    for (let i = 0; i < 3; i += 1)
      await strip.getByRole('button', { name: 'Avancer de 10 secondes' }).click();
    ...
    await nav(page, 'Plan');
    await expect(page.getByText(/^Hors champ/).first()).toBeVisible();
    ...
    await expect(
      page.locator('.space, .mobile-space').getByText('Débranchement volontaire').first(),
    ).toBeVisible();
    ...
    await form2
      .getByLabel(/^Acteur/)
      .selectOption({ label: 'Ana Sorel — Responsable de fermeture' });
    ...
    await expect(page.getByRole('meter', { name: 'Pression' })).toHaveAttribute(
      'aria-valuenow',
      '3',
    );
    ...
    await nav(page, 'Version');
    await page.getByRole('tab', { name: 'Version' }).click();
    await expect(page.getByRole('tabpanel').getByText(/1\/5/).first()).toBeVisible();

    // 10. Import invalide rejeté atomiquement, puis import valide ---------------------------------
    await openMenu(page, /^Sauvegardes/);
    const again = page.getByRole('dialog', { name: 'Sauvegardes' });
    await again
      .getByLabel('Ou coller le JSON')
      .fill('{"kind":"la-version-acceptable-save","formatVersion":9}');
  ...
  test('version complète menant à une fin existante, sans recours au desktop', async ({ page }) => {
    ...
    for (const [slot, hyp] of [
      ...
      const form = page.getByRole('dialog', {
        name: new RegExp(`Hypothèse — ${escapeRegExp(slot)}`),
      });
  ...
  test('les fonctions essentielles au clavier seul (mobile)', async ({ page }) => {
    ...
    await page
      .getByRole('button', { name: 'Choisir une hypothèse pour « Bruit de la réserve »' })
      .focus();

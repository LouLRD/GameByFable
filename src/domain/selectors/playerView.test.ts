...
function forbiddenTexts(state: ReturnType<typeof run>): string[] {
  ...
  for (const s of scenario.data.statements)
    if (!unlockedStatements.has(s.id)) out.push(s.publicText);
  const unlockedEvidence = new Set(state.unlockedEvidenceIds);
  for (const e of scenario.data.evidence)
    if (!unlockedEvidence.has(e.id)) out.push(e.playerText, e.label);
...
describe('vue joueur : sécurité narrative', () => {
  it('au départ, aucune information canonical-only ne traverse le sélecteur', () => {
    ...
    expect(json).not.toMatch(
      /bouilloire|justificatif rose|derrière la fiche|paper-only|protect-ana/i,
    );
  });
  ...
  it('en cours de partie, seules les révélations obtenues apparaissent', () => {
    ...
    expect(view.facts.some((f) => f.id === 'f_pallet_placed' && f.degree === 'established')).toBe(
      true,
    );
    expect(view.facts.some((f) => f.id === 'f_sleeve_on_trolley' && f.degree === 'reported')).toBe(
      true,
    );
  ...
  it('les perceptions d’un personnage ne sont visibles qu’une fois révélées', () => {
    const before = selectPlayerView(
      scenario,
      run([confront('jo', 's_jo_initial', 'e_camera_gap')]),
    );
    expect(before.characters.find((c) => c.id === 'noe')?.perceptions).toEqual([]);
    const after = selectPlayerView(
      scenario,
      run([
        confront('jo', 's_jo_initial', 'e_camera_gap'),
  ...
  it('l’onboarding progresse sans bloquer', () => {
    ...
    expect(selectPlayerView(scenario, s1, { selectedId: 'e_camera_gap' }).onboarding?.id).toBe(
      'o2',
    );
    expect(selectPlayerView(scenario, s1, { selectedId: null }).onboarding?.id).toBe('o3');
    const s2 = run(
      [
        { type: 'dismiss-onboarding', onboardingId: 'o2' as never },
        { type: 'dismiss-onboarding', onboardingId: 'o3' as never },
        claim('cash_origin', 'h_malik_theft'),
      ],
      s1,
    );

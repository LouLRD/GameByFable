...
export function matchEnding(
  ...
  adhesion: readonly SignatureDecision[],
): EndingMatch {
  const failed: string[] = [];
  ...
  if (ending.forbidsEvidenceInReport.some((e) => attached.has(e)))
    failed.push('forbidden-in-report');
  if (ending.forbidsEvidenceUnlocked.some((e) => unlocked.has(e)))
    failed.push('forbidden-unlocked');
  if (
    ending.maxBlockingContradictions !== null &&
    blocking.length > ending.maxBlockingContradictions
  )
    failed.push('blocking');

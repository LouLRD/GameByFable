...
describe('recevabilité des confrontations (échecs informatifs)', () => {
  ...
  it('personne, cible et appui inconnus ou verrouillés', () => {
    expect(findConfrontation(scenario, s0, C('zed'), 's_ana_initial', undefined)).toMatchObject({
      ok: false,
      error: { code: 'unknown-character' },
    });
    expect(findConfrontation(scenario, s0, C('ana'), 'nope', undefined)).toMatchObject({
      ok: false,
      error: { code: 'unknown-target' },
    });
    expect(
      findConfrontation(scenario, s0, C('malik'), 's_malik_clarified', undefined),
    ).toMatchObject({ ok: false, error: { code: 'target-locked' } });
    expect(
      findConfrontation(scenario, s0, C('ana'), 's_ana_initial', 'e_drawer_log'),
    ).toMatchObject({ ok: false, error: { code: 'support-locked' } });
  });
  ...
  it('la variante gardée s’applique quand l’approche directe ne libère pas de précision', () => {
    const s = run([confront('jo', 's_jo_initial', 'e_camera_gap')]);
    const direct = resolveConfrontation(
      scenario,
      s,
      C('ines'),
      's_ines_initial',
    ...
    const soft = resolveConfrontation(
      scenario,
      s,
      C('ines'),
      's_ines_initial',
    ...
    expect(direct.ok && direct.resolution.learnedPropositionIds).toContain(
      'prop_pallet_present_2056',
    );
  });
});
...
describe('sondages', () => {
  ...
  it('la personne visée refuse ; l’approche directe lui coûte de la confiance', () => {
    ...
    const after = run([
      { type: 'probe', characterId: C('malik'), targetId: 'h_malik_theft', approach: 'direct' },
    ]);
  ...
  it('réaction neutre quand rien de connu ne s’y oppose, refus fondé sur une certitude révélée', () => {
    const s0 = createInitialState(scenario);
    expect(probe(scenario, s0, C('jo'), 'h_counting_error', 'neutral')).toMatchObject({
      ok: true,
    ...
    expect(probe(scenario, s, C('ines'), 'h_scheduled_reboot', 'neutral')).toMatchObject({
      ok: true,
      result: { stance: 'refuses' },
    });
    // mais elle ne se trahit pas tant que rien n'est révélé
    expect(probe(scenario, s0, C('ines'), 'h_scheduled_reboot', 'neutral')).toMatchObject({
      ok: true,
      result: { stance: 'neutral' },
    });
  });
  ...
  it('une pièce établissant ses propres gestes est reconnue ; une pièce sans lien laisse indifférent', () => {
    const s = run([confront('ana', 's_ana_initial', 'e_till_report', 'empathetic')]);
    expect(probe(scenario, s, C('ana'), 'e_drawer_log', 'neutral')).toMatchObject({
      ok: true,
      result: { stance: 'acknowledges' },
    });
    expect(probe(scenario, s, C('noe'), 'e_drawer_log', 'neutral')).toMatchObject({
      ok: true,
      result: { stance: 'unknown' },
    });
    expect(probe(scenario, s, C('noe'), 'e_hidden_receipt', 'neutral')).toMatchObject({
      ok: false,
      error: { code: 'evidence-locked' },
    });
    expect(probe(scenario, s, C('noe'), 'nothing', 'neutral')).toMatchObject({
      ok: false,
      error: { code: 'unknown-target' },
    });
  });
  ...
  it('un sondage ne consomme pas de pression et ne débloque rien', () => {
    ...
    const probeAction: PlayerAction = {
      type: 'probe',
      characterId: C('mina'),
      targetId: 'h_mina_theft',
      approach: 'neutral',
    };

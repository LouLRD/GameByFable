...
export const sensoryDetector: ContradictionDetector & {
  detect(ctx: EvaluationContext): Contradiction[];
} = {
  id: 'sensory',
  detect(ctx: EvaluationContext): Contradiction[] {
    const out: Contradiction[] = [];
    const soundClaims = ctx.claimEvents.filter(
      (ev) => ev.extension?.worldEffect?.type === 'sound' && ev.zoneId && ev.interval,
    );
    ...
    for (const s of ctx.standingStatements) {
      ...
      if (sem.modality === 'visual') {
        ...
        const steps: ExplanationStep[] = [
          ...
          {
            type: 'sight',
            observer: sem.observerId,
            from,
            to: sem.target.zoneId,
            at: sec(mid),
            quality: sight.quality,
            via: sight.via,
            ...(sight.blockedBy
              ? { occludedBy: sight.blockedBy }
              : sight.occludedBy[0]
                ? { occludedBy: sight.occludedBy[0] }
                : {}),
          },
        ];
        ...
        if (sight.fidelity === 'none') {
          ...
          steps.push({
            type: 'conclusion',
            text: `Aucune ligne de vue n'atteint la zone depuis ${zoneLabel(ctx.scenario, from)} à cet instant.`,
          });
        ...
        if (sem.target.characterId) {
          ...
          if (occ.status === 'impossible') {
            ...
            title =
              title ||
              `${observerName} n'a pas pu voir ${characterName(ctx.scenario, sem.target.characterId)} à ${zoneLabel(ctx.scenario, sem.target.zoneId)}`;
            for (const c of occ.conflicts) steps.push(...c.steps);
            steps.push({
              type: 'conclusion',
              text: `${characterName(ctx.scenario, sem.target.characterId)} est établi·e ailleurs : ce qui a été vu ne pouvait pas être cette personne.`,
            });
            ...
            steps.push({
              type: 'conclusion',
              text: `La personne désignée déclare avoir été ailleurs : perception et témoignage divergent.`,
            });
          }
        }
        if (severity) {
          out.push(
            makeContradiction({
              ...
              involvedIds: [
                s.id,
                sem.observerId,
                ...(sem.target.characterId ? [sem.target.characterId] : []),
              ],
              explanation: steps,
              inspectableAt: mid,
              inspectableZoneIds: [from, sem.target.zoneId],
              suggestedEvidenceIds: [
                ...relatedEvidence(ctx, [def.id]),
                ...(sight.occludedBy.length > 0 || sight.blockedBy
                  ? ctx.unlockedEvidence
                      .filter((e) =>
                        ctx.scenario.data.obstructions.some(
                          (o) => o.publicAfterEvidenceId === e.id,
                        ),
                      )
                      .map((e) => e.id)
                  : []),
      ...
      const soundTags =
        sem.modality === 'audio' ? sem.target.claimedTags : (sem.target.soundTags ?? []);
      if (soundTags.length > 0) {
        for (const ev of soundClaims) {
          ...
          const heard = hearSignal(
            from,
            {
              originZoneId: ev.zoneId,
          ...
          if (heard.fidelity === 'none') {
            severity = 'critical';
            problems.push(
              `Le son proposé n'est pas audible depuis ${zoneLabel(ctx.scenario, from)} : intensité perçue ${heard.intensity.toFixed(2)}.`,
            );
          }
          ...
          if (shared.length / soundTags.length < 0.5) {
            ...
            steps.push({
              ...
              expected: soundTags,
              claimed: effect.signatureTags,
              shared,
            });
            problems.push(
              `La signature du son proposé ne correspond pas à ce que ${observerName} décrit.`,
            );
          }
          const tolerant = interval(
            ev.interval.start - TIMING_TOLERANCE,
            ev.interval.end + TIMING_TOLERANCE,
          );
          if (!intervalsOverlap(tolerant, sem.target.interval)) {
            severity = severity ?? 'major';
            steps.push({
              type: 'timing-mismatch',
              expected: sem.target.interval,
              claimed: ev.interval,
            });
          ...
          out.push(
            makeContradiction({
              ...
              title:
                heard.fidelity === 'none'
                  ? `Son inaudible depuis ${zoneLabel(ctx.scenario, from)}`
                  : `Le bruit proposé ne correspond pas à la perception de ${observerName}`,
              ruleId: RULE_SOUND,

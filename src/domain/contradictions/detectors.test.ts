...
import { scenario, run, claim, confront, CANONICAL_CONFRONTATIONS, CANONICAL_CLAIMS } from '@/test/helpers';
...
const ev = (actions: Parameters<typeof run>[0]) => evaluateVersion(scenario, run(actions)).evaluation;
...
describe('les sept familles de contradiction', () => {
  ...
  it('physique : trajet trop long depuis la dernière position établie ; corriger l’heure la fait disparaître', () => {
    const bad = ev([claim('video_outage', 'h_deliberate_unplug', { actorId: 'malik' as never, interval: interval(503, 513) })]);
    const c = bad.coherence.blocking.find((x) => x.kind === 'physical');
    expect(c).toBeDefined();
    const steps = renderExplanation(c?.explanation ?? [], scenario).map((s) => s.text).join(' ');
    expect(steps).toMatch(/Trajet Rayon 2 → Bureau/);
    expect(steps).toMatch(/ne peut atteindre/);
    const good = ev([claim('video_outage', 'h_deliberate_unplug', { actorId: 'malik' as never, interval: interval(520, 540) })]);
    ...
    const ana = ev([claim('video_outage', 'h_deliberate_unplug', { actorId: 'ana' as never, interval: interval(503, 513) })]);
    expect(ana.coherence.blocking.filter((x) => x.kind === 'physical' || x.kind === 'temporal')).toEqual([]);
  });

  it('sensorielle : vue partielle puis identification impossible après la révélation de la palette', () => {
    const start = ev([]);
    const before = start.contradictions.find((c) => c.kind === 'sensory' && c.involvedIds.includes('s_noe_initial'));
    expect(before?.severity).toBe('notice');
    const after = ev([confront('jo', 's_jo_initial', 'e_camera_gap')]).contradictions.find((c) => c.kind === 'sensory' && c.involvedIds.includes('s_noe_initial'));
    ...
    const text = renderExplanation(after?.explanation ?? [], scenario).map((s) => s.text).join(' ');
  ...
  it('sensorielle : une source sonore incompatible avec la perception décrite', () => {
    ...
    const c = e.coherence.blocking.find((x) => x.kind === 'sensory' && x.involvedIds.includes('h_freezer_alarm'));
    expect(c).toBeDefined();
    const text = renderExplanation(c?.explanation ?? [], scenario).map((s) => s.text).join(' ');
    ...
    const s = run([confront('jo', 's_jo_initial', 'e_camera_gap'), confront('noe', 's_noe_initial', 'e_pallet_scan'), claim('noise_source', 'h_trolley_threshold')]);
  ...
  it('matérielle : une pièce jointe exclut une hypothèse ; la retirer du rapport la transforme en omission', () => {
    const s = run([confront('jo', 's_jo_initial', 'e_camera_gap'), confront('ines', 's_ines_initial', 'e_pallet_scan', 'direct'), claim('video_outage', 'h_deliberate_unplug', { actorId: 'ana' as never })]);
    const e = evaluateVersion(scenario, s).evaluation;
    const c = e.coherence.blocking.find((x) => x.kind === 'material' && x.involvedIds.includes('e_breaker_log'));
    expect(c?.severity).toBe('critical');
    const detached = run([{ type: 'set-evidence-attached', evidenceId: 'e_breaker_log' as never, attached: false }], s);
    ...
    const omission = e2.contradictions.find((x) => x.kind === 'material' && x.title.startsWith('Omission'));
  ...
  it('matérielle : règle générique « la palette n’explique pas l’absence de fichier »', () => {
    ...
    expect(c).toBeDefined();
    expect(renderExplanation(c?.explanation ?? [], scenario).some((s) => s.text.includes('redémarrage de session'))).toBe(true);
  });
  ...
  it('épistémique : une déclaration sans chemin de connaissance connu est signalée, puis résolue', () => {
    ...
    const after = ev([confront('jo', 's_jo_initial', 'e_camera_gap'), confront('noe', 's_noe_initial', 'e_pallet_scan')]);
  ...
  it('discursive : la version contredit une déclaration debout (majeure) ; une déclaration discréditée ne bloque plus', () => {
    ...
    const s = run([confront('jo', 's_jo_initial', 'e_camera_gap'), confront('ines', 's_ines_initial', 'e_pallet_scan', 'direct'), claim('video_outage', 'h_circuit_overload')]);
    const e = evaluateVersion(scenario, s).evaluation;
    const c = e.coherence.blocking.find((x) => x.kind === 'discursive' && x.involvedIds.includes('s_ines_initial'));
    ...
    const d = e2.contradictions.find((x) => x.kind === 'discursive' && x.involvedIds.includes('s_malik_initial') && x.involvesVersion);
    expect(d?.severity).toBe('notice');
  });

  it('discursive : deux témoignages incompatibles sont signalés sans compter pour la cohérence', () => {
    const e = ev([]);
    const c = e.contradictions.find((x) => x.kind === 'discursive' && x.involvedIds.includes('s_ines_initial') && x.involvedIds.includes('s_noe_initial'));
...
describe('cycle de vie des contradictions', () => {
  it('retirer une claim supprime les contradictions qui dépendent uniquement d’elle', () => {
    const withClaim = run([claim('video_outage', 'h_deliberate_unplug', { actorId: 'malik' as never })]);
    ...
    const independent = e1.contradictions.filter((c) => !c.involvesVersion).map((c) => c.id).sort();
    expect(e2.contradictions.filter((c) => !c.involvesVersion).map((c) => c.id).sort()).toEqual(independent);
  });

  it('les identifiants sont stables et la déduplication conserve la sévérité la plus haute', () => {
    const a = makeContradiction({ kind: 'material', severity: 'notice', title: 'a', ruleId: 'r', involvedIds: ['x', 'y'], explanation: [], involvesVersion: true });
    const b = makeContradiction({ kind: 'material', severity: 'critical', title: 'b', ruleId: 'r', involvedIds: ['y', 'x'], explanation: [], involvesVersion: false, suggestedEvidenceIds: ['e_till_report' as never] });
  ...
  it('les détecteurs distinguent inconnu / non étayé / contredit / impossible', () => {
    const s = run([
      ...
      claim('video_outage', 'h_deliberate_unplug', { actorId: 'jo' as never, interval: interval(520, 540) }), // inconnu : Jo hors champ
  ...
  it('propositionsConflict détecte exclusions explicites et conflits spatio-temporels', () => {
    ...
    expect(propositionsConflict(p('prop_refund_happened')!, p('prop_counting_error')!)).toBe('explicit');
    expect(propositionsConflict(p('prop_ines_cold_aisle_continuous')!, p('prop_kettle_caused_trip')!)).not.toBeNull();
    expect(propositionsConflict(p('prop_customer_left_2052')!, p('prop_freezer_alarm')!)).toBeNull();
  });
});

describe('aucun spoiler avant révélation', () => {
  const hiddenLabels = () =>
    scenario.data.canonicalFacts.filter((f) => f.secrecy === 'canonical-only').map((f) => scenario.index.factPresentations.get(f.id)?.label ?? '');
  const hiddenStatementTexts = () => scenario.data.statements.filter((s) => !s.availableAtStart).map((s) => s.publicText);

  it('les explications ne citent ni fait secret ni déclaration non débloquée', () => {
    const e = ev([claim('cash_origin', 'h_malik_theft'), claim('video_outage', 'h_deliberate_unplug', { actorId: 'malik' as never }), claim('noise_source', 'h_stockroom_door', { actorId: 'ines' as never })]);
    const texts = [...e.contradictions, ...e.motivational].flatMap((c) => [c.title, ...renderExplanation(c.explanation, scenario).map((s) => s.text)]).join('\n');

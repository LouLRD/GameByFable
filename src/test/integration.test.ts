...
describe('chaînes complètes', () => {
  it('claim → simulation → contradiction → explication', () => {
    ...
    expect(c?.steps.map((st) => st.text).join(' ')).toMatch(
      /Trajet Rayon 2 → Bureau en partant à 20 h 57 min 20 s : 16 s/,
    );
  ...
  it('confrontation → révélation → nouvelle option', () => {
    const s0 = run([]);
    expect(
      selectPlayerView(scenario, s0).hypotheses.some((h) => h.id === 'h_circuit_overload'),
    ).toBe(false);
    const s1 = run(
      [
    ...
    expect(view.evidence.map((e) => e.id)).toEqual(
      expect.arrayContaining(['e_pallet_scan', 'e_breaker_log', 'e_warm_kettle']),
    );
  ...
  it('export/import : état sémantiquement équivalent, ordre des actions préservé', () => {
    const actions: PlayerAction[] = [
      ...CANONICAL_CONFRONTATIONS.slice(0, 3),
      claim('video_outage', 'h_circuit_overload'),
  ...
  it('importer une sauvegarde invalide laisse la sauvegarde courante intacte', () => {
    ...
    const save = createSaveFile({
      ...
      seed: s.seed,
      actions: [claim('cash_origin', 'h_counting_error')],
      ui: { cursor: 0, selectedId: null, activeSpace: null },
      label: 'ok',
      savedAt: '2026-09-04T21:12:00.000Z',
      appVersion: '1.0.0',
    });
    repo.write('slot-1', save);
    const bad = parseImport('{"kind":"la-version-acceptable-save","formatVersion":9}', {
      scenarioId: s.scenarioId,
      scenarioVersion: s.scenarioVersion,
    });
    expect(bad.ok).toBe(false);
    if (!bad.ok) expect(bad.reason).toBe('newer-format');
    const other = parseImport(JSON.stringify({ ...save, scenarioId: 'autre-scenario' }), {
      scenarioId: s.scenarioId,
      scenarioVersion: s.scenarioVersion,
    });
  ...
  it('les actions rejetées au replay sont signalées sans casser la partie', () => {
    const envelope = toEnvelope(createInitialState(scenario), [
      claim('cash_origin', 'h_emergency_refund'),
  ...
  it('modifier le temps d’une claim de trente secondes change réellement les calculs', () => {
    const a = run([
      claim('video_outage', 'h_deliberate_unplug', {
        actorId: 'malik' as never,
        interval: interval(490, 520),
      }),
    ]);
    const b = run([
      claim('video_outage', 'h_deliberate_unplug', {
        actorId: 'malik' as never,
        interval: interval(520, 550),
      }),
    ]);
    ...
    expect(
      renderExplanation(ea.coherence.blocking[0]?.explanation ?? [], scenario).length,
    ).toBeGreaterThan(0);
    const r = applyAction(
      scenario,
      a,

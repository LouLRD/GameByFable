...
function checkUnique(issues: Issues, ns: string, ids: string[]): Set<string> {
  ...
  ids.forEach((v, i) => {
    if (seen.has(v))
      err(issues, 'duplicate-id', `${ns}[${i}]`, `Identifiant dupliqué « ${v} » dans ${ns}.`);
    seen.add(v);
  });
  return seen;
}
...
export function validateScenario(raw: RawScenario, ext: RawExtension): Issues {
  ...
  const zoneIds = checkUnique(
    issues,
    'zones',
    raw.zones.map((z) => z.id),
  );
  checkUnique(
    issues,
    'passages',
    raw.passages.map((p) => p.id),
  );
  const obstructionIds = checkUnique(
    issues,
    'obstructions',
    raw.obstructions.map((o) => o.id),
  );
  const characterIds = checkUnique(
    issues,
    'characters',
    raw.characters.map((c) => c.id),
  );
  const factIds = checkUnique(
    issues,
    'canonicalFacts',
    raw.canonicalFacts.map((f) => f.id),
  );
  checkUnique(
    issues,
    'soundSignals',
    raw.soundSignals.map((s) => s.id),
  );
  const perceptionIds = checkUnique(
    issues,
    'perceptions',
    raw.perceptions.map((p) => p.id),
  );
  checkUnique(
    issues,
    'initialBeliefs',
    raw.initialBeliefs.map((b) => b.id),
  );
  const evidenceIds = checkUnique(
    issues,
    'evidence',
    raw.evidence.map((e) => e.id),
  );
  const statementIds = checkUnique(
    issues,
    'statements',
    raw.statements.map((s) => s.id),
  );
  const slotIds = checkUnique(
    issues,
    'claimSlots',
    raw.claimSlots.map((s) => s.id),
  );
  const hypothesisIds = checkUnique(
    issues,
    'hypotheses',
    raw.hypotheses.map((h) => h.id),
  );
  const confrontationIds = checkUnique(
    issues,
    'confrontations',
    raw.confrontations.map((c) => c.id),
  );
  checkUnique(
    issues,
    'genericRules',
    raw.genericRules.map((r) => r.id),
  );
  checkUnique(
    issues,
    'pressureRewards',
    raw.pressureRewards.map((r) => r.id),
  );
  const endingIds = checkUnique(
    issues,
    'endings',
    raw.endings.map((e) => e.id),
  );
  checkUnique(
    issues,
    'onboarding',
    raw.onboarding.map((o) => o.id),
  );
  const propositionIds = checkUnique(
    issues,
    'extension.propositions',
    ext.propositions.map((p) => p.id),
  );
  ...
  const refChar = (path: string, v: string) => {
    if (!characterIds.has(v))
      err(issues, 'unknown-character', path, `Personnage inconnu « ${v} ».`);
  };
  const refProp = (path: string, v: string) => {
    if (!propositionIds.has(v))
      err(issues, 'unknown-proposition', path, `Proposition non définie « ${v} ».`);
  };
  ...
  const refStatement = (path: string, v: string) => {
    if (!statementIds.has(v))
      err(issues, 'unknown-statement', path, `Déclaration inconnue « ${v} ».`);
  };
  const refHypothesis = (path: string, v: string) => {
    if (!hypothesisIds.has(v))
      err(issues, 'unknown-hypothesis', path, `Hypothèse inconnue « ${v} ».`);
  };
  const refConfrontation = (path: string, v: string) => {
    if (!confrontationIds.has(v))
      err(issues, 'unknown-confrontation', path, `Confrontation inconnue « ${v} ».`);
  };
  ...
  const checkInterval = (path: string, i: { start: number; end: number }) => {
    if (i.start < 0 || i.end > duration || i.start > i.end) {
      err(
        issues,
        'interval-out-of-window',
        path,
        `Intervalle [${i.start}, ${i.end}] hors fenêtre [0, ${duration}].`,
      );
    }
  };
  ...
  raw.passages.forEach((p, i) => {
    ...
    if (p.from === p.to)
      err(
        ...
        'self-passage',
        `passages[${i}]`,
        `Le passage « ${p.id} » relie une zone à elle-même.`,
      );
    if (p.affectedByObstructionId && !obstructionIds.has(p.affectedByObstructionId)) {
      err(
        issues,
        'unknown-obstruction',
        `passages[${i}].affectedByObstructionId`,
        `Obstruction inconnue « ${p.affectedByObstructionId} ».`,
      );
    }
    ...
    raw.zones.forEach((z) => {
      if (!seen.has(z.id))
        err(
          ...
          'disconnected-zone',
          `zones.${z.id}`,
          `La zone « ${z.label} » n'est reliée à aucune autre.`,
        );
    });
  }
  raw.obstructions.forEach((o, i) => {
    ...
    if (o.publicAfterEvidenceId)
      refEvidence(`obstructions[${i}].publicAfterEvidenceId`, o.publicAfterEvidenceId);
    ...
    if (!affected && !blocks)
      warn(
        ...
        'inert-obstruction',
        `obstructions[${i}]`,
        `L'obstruction « ${o.id} » n'affecte ni chemin ni ligne de vue.`,
      );
  });

  // --- personnages -----------------------------------------------------------
  raw.characters.forEach((c, i) => {
    if (!Number.isInteger(c.initialTrust))
      err(
        issues,
        'invalid-trust',
        `characters[${i}].initialTrust`,
        'Confiance initiale non entière.',
      );
    ...
    if (!raw.signatureRules.some((r) => r.characterId === c.id)) {
      err(
        ...
        `characters[${i}]`,
        `Aucune règle de signature pour « ${c.name} ».`,
      );
    }
  });
  raw.movementTracks.forEach((t, i) => {
    ...
    t.segments.forEach((sgm, j) => {
      ...
      if (sgm.start < prevEnd)
        err(
          issues,
          'track-overlap',
          `movementTracks[${i}].segments[${j}]`,
          `Segments qui se chevauchent pour « ${t.characterId} ».`,
        );
      prevEnd = sgm.end;
    });
  });
  ...
  raw.canonicalFacts.forEach((f, i) => {
    ...
    if (f.variableSlotId && !slotIds.has(f.variableSlotId))
      err(
        issues,
        'unknown-slot',
        `canonicalFacts[${i}].variableSlotId`,
        `Slot inconnu « ${f.variableSlotId} ».`,
      );
    if (f.canonicalHypothesisId) {
      ...
      if (h && f.variableSlotId && h.slotId !== f.variableSlotId) {
        err(
          ...
          'slot-mismatch',
          `canonicalFacts[${i}]`,
          `L'hypothèse canonique « ${h.id} » n'appartient pas au slot « ${f.variableSlotId} ».`,
        );
      }
    }
    ...
    if (f.secrecy === 'canonical-only' && ('playerText' in record || 'label' in record)) {
      err(
        ...
        'secret-player-text',
        `canonicalFacts[${i}]`,
        `Le fait secret « ${f.id} » porte un texte joueur.`,
      );
    }
  });
  ...
  const knownProvenance = new Set<string>([
    ...factIds,
    ...perceptionIds,
    ...evidenceIds,
    ...statementIds,
  ...
  raw.initialBeliefs.forEach((b, i) => {
    ...
    b.provenanceIds.forEach((pid, j) => {
      if (!knownProvenance.has(pid))
        err(
          issues,
          'unknown-provenance',
          `initialBeliefs[${i}].provenanceIds[${j}]`,
          `Provenance inconnue « ${pid} ».`,
        );
    });
  });
  ...
  raw.evidence.forEach((e, i) => {
    ...
    if (!e.availableAtStart && (e.unlockBy ?? []).length === 0) {
      err(
        ...
        `evidence[${i}]`,
        `La pièce « ${e.id} » n'est ni disponible au départ ni déblocable.`,
      );
    }
  });
  raw.statements.forEach((s, i) => {
    ...
    s.unlockConditionIds.forEach((c, j) =>
      refConfrontation(`statements[${i}].unlockConditionIds[${j}]`, c),
    );
    if (!s.availableAtStart && s.unlockConditionIds.length === 0) {
      err(
        ...
        `statements[${i}]`,
        `La déclaration « ${s.id} » n'est ni disponible au départ ni déblocable.`,
      );
    }
  });
  ...
  raw.hypotheses.forEach((h, i) => {
    if (!slotIds.has(h.slotId))
      err(issues, 'unknown-slot', `hypotheses[${i}].slotId`, `Slot inconnu « ${h.slotId} ».`);
    ...
    h.unlockEvidenceIds.forEach((e, j) =>
      refEvidence(`hypotheses[${i}].unlockEvidenceIds[${j}]`, e),
    );
    const available = h.availableAtStart ?? h.unlockEvidenceIds.length === 0;
    if (!available && h.unlockEvidenceIds.length === 0) {
      err(
        issues,
        'unreachable-hypothesis',
        `hypotheses[${i}]`,
        `L'hypothèse « ${h.id} » n'est ni disponible ni déblocable.`,
      );
    }
    if (h.availableAtStart === true && h.unlockEvidenceIds.length > 0) {
      err(
        issues,
        'hypothesis-unlock-conflict',
        `hypotheses[${i}]`,
        `« ${h.id} » est disponible au départ ET conditionnée par des pièces.`,
      );
    }
    if (h.requiresActor && h.defaultActorId) {
      warn(
        issues,
        'actor-default-with-required',
        `hypotheses[${i}]`,
        `« ${h.id} » exige un acteur mais en propose un par défaut.`,
      );
    }
  });
  raw.claimSlots.forEach((slot, i) => {
    if (!raw.hypotheses.some((h) => h.slotId === slot.id))
      err(
        issues,
        'empty-slot',
        `claimSlots[${i}]`,
        `Aucune hypothèse pour le slot « ${slot.id} ».`,
      );
  });
    ...
    c.supportIds.forEach((t, j) => {
      if (!pieceIds.has(t))
        err(
          issues,
          'unknown-piece',
          `confrontations[${i}].supportIds[${j}]`,
          `Pièce d'appui inconnue « ${t} ».`,
        );
    });
    (['neutral', 'empathetic', 'direct'] as const).forEach((a) => {
      c.approaches[a].unlockEvidenceIds.forEach((e, j) =>
        refEvidence(`confrontations[${i}].approaches.${a}.unlockEvidenceIds[${j}]`, e),
      );
      c.approaches[a].unlockStatementIds.forEach((s, j) =>
        refStatement(`confrontations[${i}].approaches.${a}.unlockStatementIds[${j}]`, s),
      );
    });
  });
  ...
  raw.genericRules.forEach((r, i) => {
    ...
    if (r.conflictsWithEvidenceId)
      refEvidence(`genericRules[${i}].conflictsWithEvidenceId`, r.conflictsWithEvidenceId);
    if (r.ifProposition) refProp(`genericRules[${i}].ifProposition`, r.ifProposition);
    if (r.excludeProposition)
      refProp(`genericRules[${i}].excludeProposition`, r.excludeProposition);
    r.requireAnyProposition?.forEach((p, j) =>
      refProp(`genericRules[${i}].requireAnyProposition[${j}]`, p),
    );
    if (r.statementId) refStatement(`genericRules[${i}].statementId`, r.statementId);
  });
  raw.pressureRewards.forEach((r, i) => {
    if (r.condition.type === 'evidence-unlocked')
      r.condition.all.forEach((e, j) =>
        refEvidence(`pressureRewards[${i}].condition.all[${j}]`, e),
      );
  });
  raw.endings.forEach((e, i) => {
    e.requiresHypotheses.forEach((h, j) =>
      refHypothesis(`endings[${i}].requiresHypotheses[${j}]`, h),
    );
    e.requiresEvidence.forEach((v, j) => refEvidence(`endings[${i}].requiresEvidence[${j}]`, v));
    e.forbidsEvidenceInReport.forEach((v, j) =>
      refEvidence(`endings[${i}].forbidsEvidenceInReport[${j}]`, v),
    );
    e.forbidsEvidenceUnlocked.forEach((v, j) =>
      refEvidence(`endings[${i}].forbidsEvidenceUnlocked[${j}]`, v),
    );
    if (e.specialSignatureRule) {
      const m = /^(.+)-never-signs$/.exec(e.specialSignatureRule);
      if (!m || !characterIds.has(m[1] ?? ''))
        err(
          issues,
          'unknown-special-rule',
          `endings[${i}].specialSignatureRule`,
          `Règle spéciale non reconnue « ${e.specialSignatureRule} ».`,
        );
    }
    ...
    if (slotsCovered.size !== e.requiresHypotheses.length) {
      err(
        issues,
        'ending-slot-collision',
        `endings[${i}]`,
        `La fin « ${e.id} » exige deux hypothèses du même slot.`,
      );
    }
  });
  if (!raw.endings.some((e) => e.fallback))
    err(issues, 'no-fallback-ending', 'endings', 'Aucune fin de repli.');
  raw.signatureRules.forEach((r, i) => {
    refChar(`signatureRules[${i}].characterId`, r.characterId);
    r.rejectsPropositions.forEach((p, j) =>
      refProp(`signatureRules[${i}].rejectsPropositions[${j}]`, p),
    );
  });
  ...
  const reachableEvidence = new Set(
    raw.evidence.filter((e) => e.availableAtStart).map((e) => e.id),
  );
  ...
  while (changed) {
    ...
    for (const c of raw.confrontations) {
      const targetOk = c.targetIds.some(
        (t) => reachableEvidence.has(t) || reachableStatements.has(t),
      );
      const supportOk =
        c.supportIds.length === 0 ||
        c.supportIds.some((t) => reachableEvidence.has(t) || reachableStatements.has(t));
  ...
  const reachableHypotheses = new Set(
    raw.hypotheses
      .filter(
        (h) =>
          (h.availableAtStart ?? h.unlockEvidenceIds.length === 0) ||
          h.unlockEvidenceIds.every((e) => reachableEvidence.has(e)),
      )
      .map((h) => h.id),
  );
  const reachableEndings = raw.endings.filter(
    (e) =>
      !e.fallback &&
      e.requiresHypotheses.every((h) => reachableHypotheses.has(h)) &&
      e.requiresEvidence.every((v) => reachableEvidence.has(v)),
  );
  if (reachableEndings.length === 0)
    err(issues, 'no-reachable-ending', 'endings', "Aucune fin n'est structurellement atteignable.");
  raw.evidence.forEach((e) => {
    if (!reachableEvidence.has(e.id))
      warn(
        issues,
        'unreachable-evidence-chain',
        `evidence.${e.id}`,
        `La pièce « ${e.id} » n'est jamais déblocable par une chaîne de confrontations.`,
      );
  });
  ...
  ext.propositions.forEach((p, i) => {
    ...
    Object.keys(p.costKeys).forEach((c) =>
      refChar(`extension.propositions[${i}].costKeys.${c}`, c),
    );
    ...
    switch (sem.type) {
      ...
      case 'perceived':
        ...
        if (sem.observerZoneId)
          refZone(`extension.propositions[${i}].semantics.observerZoneId`, sem.observerZoneId);
        if (sem.target.characterId)
          refChar(
            `extension.propositions[${i}].semantics.target.characterId`,
            sem.target.characterId,
          );
        refZone(`extension.propositions[${i}].semantics.target.zoneId`, sem.target.zoneId);
        checkInterval(
          `extension.propositions[${i}].semantics.target.interval`,
          sem.target.interval,
        );
        break;
      ...
      case 'assertion':
        if (sem.subjectId)
          refChar(`extension.propositions[${i}].semantics.subjectId`, sem.subjectId);
        break;
    }
  });
  ...
  ext.propositions.forEach((p) => {
    p.excludes.forEach((x) => {
      ...
      if (other && !other.excludes.includes(p.id))
        warn(
          issues,
          'asymmetric-exclusion',
          `extension.propositions.${p.id}`,
          `Exclusion non symétrique entre « ${p.id} » et « ${x} » (rendue symétrique au chargement).`,
        );
    });
  });
  ext.evidenceMarkers.forEach((m, i) => {
    ...
    if (m.at !== undefined && (m.at < 0 || m.at > duration))
      err(
        issues,
        'interval-out-of-window',
        `extension.evidenceMarkers[${i}].at`,
        'Instant hors fenêtre.',
      );
  });
  ext.facts.forEach((f, i) => {
    refFact(`extension.facts[${i}].factId`, f.factId);
    f.revealedByEvidenceIds.forEach((e, j) =>
      refEvidence(`extension.facts[${i}].revealedByEvidenceIds[${j}]`, e),
    );
    f.reportedByStatementIds.forEach((s, j) =>
      refStatement(`extension.facts[${i}].reportedByStatementIds[${j}]`, s),
    );
    f.revealedByConfrontationIds.forEach((c, j) =>
      refConfrontation(`extension.facts[${i}].revealedByConfrontationIds[${j}]`, c),
    );
  });
  raw.canonicalFacts.forEach((f) => {
    if (!ext.facts.some((x) => x.factId === f.id))
      err(
        issues,
        'missing-fact-label',
        `extension.facts`,
        `Aucun libellé joueur pour le fait « ${f.id} ».`,
      );
  });
  ...
  ext.statements.forEach((s, i) => {
    ...
    s.revealsPerceptionIds.forEach((p, j) => {
      if (!perceptionIds.has(p))
        err(
          issues,
          'unknown-perception',
          `extension.statements[${i}].revealsPerceptionIds[${j}]`,
          `Perception inconnue « ${p} ».`,
        );
    });
  });
  ext.confrontations.forEach((c, i) => {
    refConfrontation(`extension.confrontations[${i}].confrontationId`, c.confrontationId);
    c.retractsStatementIds.forEach((s, j) =>
      refStatement(`extension.confrontations[${i}].retractsStatementIds[${j}]`, s),
    );
    Object.keys(c.admitsCostKeys).forEach((k) =>
      refChar(`extension.confrontations[${i}].admitsCostKeys.${k}`, k),
    );
  ...
  raw.confrontations.forEach((c) => {
    if (!ext.confrontations.some((x) => x.confrontationId === c.id))
      err(
        issues,
        'missing-variants',
        'extension.confrontations',
        `Aucune variante de réponse pour « ${c.id} ».`,
      );
  });
  ext.characters.forEach((c, i) =>
    refChar(`extension.characters[${i}].characterId`, c.characterId),
  );
  raw.characters.forEach((c) => {
    if (!ext.characters.some((x) => x.characterId === c.id))
      err(
        issues,
        'missing-character-extension',
        'extension.characters',
        `Aucune réaction définie pour « ${c.id} ».`,
      );
  });
  ext.endings.forEach((e, i) => {
    if (!endingIds.has(e.endingId))
      err(
        issues,
        'unknown-ending',
        `extension.endings[${i}].endingId`,
        `Fin inconnue « ${e.endingId} ».`,
      );
  });
  ext.cameraCoverage.zoneIds.forEach((z, i) =>
    refZone(`extension.cameraCoverage.zoneIds[${i}]`, z),
  );
  refEvidence('extension.cameraCoverage.gapEvidenceId', ext.cameraCoverage.gapEvidenceId);
  if (
    !ext.evidenceMarkers.some(
      (m) => m.evidenceId === ext.cameraCoverage.gapEvidenceId && m.interval,
    )
  ) {
    err(
      issues,
      'camera-gap-marker',
      'extension.cameraCoverage',
      'La pièce de coupure caméra doit avoir un marqueur avec intervalle.',
    );
  }
  ...
  raw.claimSlots.forEach((slot) => {
    ...
    if (!viaFact && !viaExt)
      err(
        ...
        `claimSlots.${slot.id}`,
        `Aucune hypothèse canonique connue pour le slot « ${slot.id} ».`,
      );
  });
...
export function deriveTruth(
  ...
  for (const hid of canonicalBySlot.values()) {
    for (const p of hypById.get(hid)?.propositions ?? [])
      set(p, true, `hypothèse canonique ${hid}`);
  }
  ...
  for (const s of raw.statements) {
    ...
    if (s.relationToBelief === 'lie' || s.relationToBelief === 'embellished')
      set(s.propositionId, false, `déclaration ${s.relationToBelief} ${s.id}`);
  }
  ...
  for (const h of raw.hypotheses) {
    ...
    for (const p of h.propositions)
      if (!derived.has(p)) set(p, false, `hypothèse non canonique ${h.id}`);
  }
  ...
  for (const p of ext.propositions) {
    ...
    if (value === null || value === undefined) {
      issues.push({
        ...
        code: 'truth-unknown',
        path: `extension.propositions.${p.id}.truth`,
        message: `Vérité indéterminée pour « ${p.id} ».`,
      });
    } else {

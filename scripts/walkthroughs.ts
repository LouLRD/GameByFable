...
const A = (s: GameState, a: PlayerAction) => {
  const r = applyAction(scenario, s, a);
  if (!r.ok) {
    console.log('REFUSED', a, r.error);
    return s;
  }
  return r.state;
};
const dump = (label: string, s: GameState) => {
  ...
  console.log(
    'coherence',
    evaluation.coherence.status,
    'completeness',
    evaluation.completeness,
    'disclosure',
    evaluation.disclosure.establishedExplained.toFixed(2),
    'signatures',
    evaluation.signatureCount,
    'reachable',
    evaluation.reachableEndingIds,
  );
  for (const c of evaluation.contradictions) {
    console.log(
      ` - [${c.kind}/${c.severity}${c.involvesVersion ? '/V' : ''}] ${c.title}  {${c.involvedIds.join(',')}}`,
    );
    for (const st of renderExplanation(c.explanation, scenario)) console.log(`      · ${st.text}`);
  }
  for (const d of evaluation.adhesion)
    console.log(
      `   ${d.characterId}: ${d.verdict} — ${d.publicReasons.join(' | ')} ## ${d.internalReasons.join(' | ')}`,
    );
...
(() => {
  ...
  s = A(s, {
    ...
    hypothesisId: 'h_deliberate_unplug' as never,
    actorId: 'malik' as never,
    interval: { start: 520, end: 540 } as never,
  });
  ...
  s = A(s, {
    ...
    characterId: 'ana' as never,
    targetId: 's_ana_initial',
    supportId: 'e_drawer_log',
    approach: 'neutral',
  });
  s = A(s, {
    type: 'confront',
    characterId: 'mina' as never,
    targetId: 's_mina_initial',
    supportId: 'e_pressure_imprint',
    approach: 'empathetic',
  });
  s = A(s, {
    type: 'confront',
    characterId: 'jo' as never,
    targetId: 's_malik_clarified',
    supportId: 'e_pallet_scan',
    approach: 'neutral',
  });
  s = A(s, {
    ...
    hypothesisId: 'h_emergency_refund' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'video_outage' as never,
    hypothesisId: 'h_circuit_overload' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'receipt_path' as never,
    hypothesisId: 'h_mina_hidden_receipt' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'noise_source' as never,
    hypothesisId: 'h_trolley_threshold' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'manager_knowledge' as never,
    hypothesisId: 'h_ana_initiated_refund' as never,
  });
...
(() => {
  ...
  s = A(s, {
    type: 'set-claim',
    slotId: 'receipt_path' as never,
    hypothesisId: 'h_receipt_lost' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'noise_source' as never,
  ...
  s = A(s, {
    ...
    slotId: 'cash_origin' as never,
    hypothesisId: 'h_malik_theft' as never,
    interval: { start: 326, end: 350 } as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'video_outage' as never,
  ...
  s = A(s, {
    type: 'set-claim',
    slotId: 'receipt_path' as never,
    hypothesisId: 'h_no_receipt' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'noise_source' as never,
    hypothesisId: 'h_stockroom_door' as never,
    actorId: 'mina' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'manager_knowledge' as never,
    hypothesisId: 'h_ana_unaware' as never,
  });
  ...
  s = A(s, {
    type: 'set-claim',
    slotId: 'video_outage' as never,
    hypothesisId: 'h_scheduled_reboot' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'receipt_path' as never,
  ...
  s = A(s, {
    type: 'set-claim',
    slotId: 'noise_source' as never,
    hypothesisId: 'h_freezer_alarm' as never,
  });
  s = A(s, {
    type: 'set-claim',
    slotId: 'manager_knowledge' as never,
